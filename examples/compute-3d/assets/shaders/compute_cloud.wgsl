...
fn update(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    let location_for_noise = vec3<f32>(f32(invocation_id.x) * 0.02, f32(invocation_id.y) * 0.02, f32(invocation_id.z) * 0.02);
    let noise = simplexNoise3(location_for_noise);

    let color = vec4<f32>(noise, noise * noise, noise * 0.2, noise);
