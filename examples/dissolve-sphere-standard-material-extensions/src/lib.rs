...
use bevy::pbr::{
    ...
    MaterialPipelineKey, PBR_PREPASS_SHADER_HANDLE,
    PBR_SHADER_HANDLE,
};
use bevy::reflect::{
    std_traits::ReflectDefault, FromReflect, Reflect,
    TypeUuid,
};
use bevy::render::{
...
#[uuid = "7494888b-c082-457b-aacf-513228cc0c22"]
...
impl StandardMaterialFlags {
    ...
    const ALPHA_MODE_SHIFT_BITS: u32 =
        32 - Self::ALPHA_MODE_MASK_BITS.count_ones();
}
...
impl Material for StandardMaterial {
    fn specialize(
        ...
        if key.bind_group_data.normal_map {
                ...
                descriptor.fragment.as_mut()
            {
                fragment.shader_defs.push(
                    "STANDARDMATERIAL_NORMAL_MAP".into(),
                );
            }
        }
        ...
        if let Some(depth_stencil) =
            descriptor.depth_stencil.as_mut()
        {
            depth_stencil.bias.constant =
                key.bind_group_data.depth_bias;
        }
        Ok(())
    }
