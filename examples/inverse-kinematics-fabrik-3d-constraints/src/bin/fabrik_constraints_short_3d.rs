...
fn startup(
    ...
    let joint_1_position = Transform::from_xyz(0., 0., 3.);
    let joint_2_position = Transform::from_xyz(0., 0., 2.);
...
fn debug_transforms(query: Query<&GlobalTransform>, mut gizmos: Gizmos) {
    ...
    let base = Vec3::NEG_X * 8.;

    // gizmos.line(base, base + (Vec3::Y + Vec3::X) * 1., GREEN_400);
    // let b = (Vec3::Y + Vec3::X);
    // let a = Vec3::Y * 3.;
    // let p = b.project_onto(a);
    // gizmos.line(base, base + p, BLUE_400);
    // // gizmos.line(base, base + Vec3::Y * 3., RED_400);
}
