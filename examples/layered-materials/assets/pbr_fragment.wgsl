...
// pbr_input_from_layered_material is basically a clone of bevy_pbr/src/render/pbr_fragment.wgsl
// the modifications here are just using passed-in samplers instead of the global bindings

#define_import_path layered_materials::pbr_fragment
...
fn pbr_input_from_standard_material(
    ...
    is_front: bool,
    array_index: i32,
    base_color_texture: texture_2d_array<f32>,
    base_color_sampler: sampler,
    normal_map_texture: texture_2d_array<f32>,
    normal_map_sampler: sampler,
    depth_map: texture_2d_array<f32>,
    depth_map_sampler: sampler,
) -> pbr_types::PbrInput {
#ifdef MESHLET_MESH_MATERIAL_PASS
...
// from another file
// https://github.com/bevyengine/bevy/blob/ea8f22eb5ab93f72afcd2651089bb2926f189b04/crates/bevy_pbr/src/render/pbr_functions.wgsl#L170
...
///
/// https://github.com/bevyengine/bevy/blob/ea8f22eb5ab93f72afcd2651089bb2926f189b04/crates/bevy_pbr/src/render/parallax_mapping.wgsl#L14
...
#import bevy_pbr::{
    // pbr_bindings::{
    //     depth_map_texture, depth_map_sampler
    // },
    mesh_bindings::mesh
}
...
fn sample_depth_map(
    ...
    array_index: i32,
    depth_map_texture: texture_2d_array<f32>,
    depth_map_sampler: sampler,
) -> f32 {
...
fn parallaxed_uv(
    ...
    var texture_depth = sample_depth_map(
        uv,
    ...
    for (var i: i32 = 0; texture_depth > current_layer_depth && i <= i32(layer_count); i++) {
        ...
        uv += delta_uv;
        texture_depth = sample_depth_map(
            uv,
            material_bind_group_slot,
            array_index,
            depth_map,
    ...
    let previous_depth = sample_depth_map(
        previous_uv,
        material_bind_group_slot,
        array_index,
        depth_map,
